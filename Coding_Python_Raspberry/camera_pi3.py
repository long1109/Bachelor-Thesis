                ...
                if ((tickskiemtra2 == tickskiemtra3+2) or (tickskiemtra2 == tickskiemtra3+3) or (tickskiemtra2 == tickskiemtra3+4) or (tickskiemtra2 == tickskiemtra3+5) or (tickskiemtra2 == tickskiemtra3+6) or (tickskiemtra2 == tickskiemtra3+1) or (tickskiemtra2 == tickskiemtra3)):
                    ...
                    if ((k1 == k2) and (k3 == 1)):## GUI XE
                        ...
                        if(l3 == barcodeData_string['idnhaxe']):
                            ...
                            time.sleep(2)
                            GPIO.output(pin3,False)
                    elif ((k1 == k2) and (int(barcodeData_string['type']) == 3)):## MUONXE
                        sql_select_query4="""select * FROM users WHERE id = %s"""
                        my_cursor.execute(sql_select_query4,(barcodeData_string['idkhachhang'],))
                        result4 = my_cursor.fetchall()
                        for row4 in result4:
                            break
                        sql_select_query5="""select * FROM users WHERE id = %s"""
                        my_cursor.execute(sql_select_query5,(barcodeData_string['idkhmuon'],))
                        result5 = my_cursor.fetchall()
                        for row5 in result5:
                            break
                        nkhmuon=int(row5[3])#kiem tra chu ky khach hang muon
                        ekhmuon=int(row5[2])
                        s1khmuon=hashlib.md5(barcodeData_string['idkhmuon'].encode('us-ascii')).hexdigest()
                        kkhmuon=bin(int(binascii.hexlify(s1khmuon.encode('utf-8')),16))
                        k2khmuon=int(kkhmuon,2)
                        c1khmuon=int(barcodeData_string['rsaidkhmuon'])
                        k1khmuon=pow(c1khmuon,ekhmuon,nkhmuon)
                        lmuon = str(row4[5])
                        
                        
                        if((int(row4[6]) == 1) and (k1khmuon == k2khmuon) and (lmuon == barcodeData_string['idnhaxe'])): #NGUOI MUON LAY XE KHOI BAI
                            my_sql4 = """UPDATE users SET ghichu = %s WHERE id = %s"""
                            my_cursor.execute(my_sql4,(barcodeData_string['ghichu'],barcodeData_string['idkhachhang'],))
                            trangthai2 = 0
                            my_sql3 = """UPDATE users SET trangthai = %s WHERE id = %s"""
                            my_cursor.execute(my_sql3,(trangthai2,barcodeData_string['idkhachhang'],)) #ko xoa idnhaxe
                            #my_sql5 = """UPDATE users SET ghichu = %s WHERE id = %s"""
                            my_sql5 = """UPDATE users SET muoncuaid = %s WHERE id = %s"""
                            my_cursor.execute(my_sql5,(barcodeData_string['idkhachhang'],barcodeData_string['idkhmuon'],))
                            my_sql6 = """UPDATE users SET chomuon = %s WHERE id = %s"""
                            my_cursor.execute(my_sql6,(barcodeData_string['idkhmuon'],barcodeData_string['idkhachhang'],))
                            mydb.commit()
                            GPIO.output(pin2,True)
                            time.sleep(20)
                            GPIO.output(pin2,False)
                        elif((int(row4[6]) == 0) and (k1khmuon == k2khmuon) and (lmuon == barcodeData_string['idnhaxe'])): #nguoi muon GUI LAI XE
                            trangthai3=1
                            ticksguixem=time.time()
                            ticksguixe2m=str(int(ticksguixem))
                            ticksguixe3m=hashlib.md5(ticksguixe2m.encode('us-ascii')).hexdigest()
                            bam2m=bin(int(binascii.hexlify(ticksguixe3m.encode('us-ascii')),16))
                            bam3m=int(bam2m,2)
                            ciphertext2m=pow(bam3m,dnhaxe,nnhaxe)
                            ciphertext3m=str(ciphertext2m)
                            mm={'idnhaxe':ticksguixe2m,'rsaidnhaxe':ciphertext3m}
                            chuoiqr=json.dumps(mm)
                            q=pyqrcode.create(chuoiqr)
                            q.png('myQR',scale=3)
                            my_sql = """UPDATE users SET idnhaxe = %s WHERE id = %s"""
                            my_cursor.execute(my_sql,(ticksguixe2m,barcodeData_string['idkhachhang'],))
                            my_sql2 = """UPDATE users SET trangthai = %s WHERE id = %s"""
                            my_cursor.execute(my_sql2,(trangthai3,barcodeData_string['idkhachhang'],))
                            
                            
                            l4m=" "
                            my_sql4 = """UPDATE users SET ghichu = %s WHERE id = %s"""
                            my_cursor.execute(my_sql4,(l4m,barcodeData_string['idkhachhang'],))
                            mydb.commit()
                            imgplot=plt.imshow(mpimg.imread("myQR"))
                            plt.ion()
                            plt.show()
